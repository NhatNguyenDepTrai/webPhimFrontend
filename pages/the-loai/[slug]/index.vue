<template lang="">
  <div>
    <Title v-if="!pending && data.type">Phim  {{ data.type.name }} </Title>
  ...
  <Meta v-if="!pending && data.type" name="description" :content="data.type.meta_desc" />
   ...
   <NuxtLayout>
    <template #page>
      <div>
        <PageTitle v-if="!pending && data.type">Phim {{ data.type.name }}</PageTitle>
      </div>
      <div>
        <div class="mt-5" v-if="!pending && data && data.dataProduct.data && data.dataProduct.data.length > 0">
          <div class="flex items-center justify-between mb-3">
            <div>
              <CateTittle  v-if="!pending && data.type" class="mb-3 capitalize">Phim {{ data.type.name }} mới cập nhập</CateTittle>
            </div>
...
const { pending, data } = useFetch(config.public.apiBase + '/' + 'get-data-page-type/' + route.params.slug + '?page=1', {
  lazy: true
});
